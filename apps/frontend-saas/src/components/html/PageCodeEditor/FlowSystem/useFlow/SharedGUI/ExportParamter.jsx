...
export function ExportParamter({ id, data }) {
        ...
        <Tooltip
          ...
          title={
            <div className='p-3 text-xs text-black bg-white rounded'>
              <div className='flex items-center'>
                ...
                <div className=''>
                  <Switch
                    ...
                    onChange={(val) => {
                      //
                      updateNodeData(id, 'isExported', val)
                    }}
                  />
                </div>
              </div>
              ...
              {data.isExported && (
                <div className='flex items-center'>
                  ...
                  <div className=''>
                    <Switch
                      className='p-1 ml-2 bg-gray-200'
                      defaultChecked={data.isGroupedForExported}
                      onChange={(val) => {
                        //
                        updateNodeData(id, 'isGroupedForExported', val)
                      }}
                    />
                  </div>
                </div>
              ...
              {data.isExported && data.isGroupedForExported && (
                <div className=''>
                  <div>Group Name</div>
                  <div className=''>
                    <Input
                      ...
                      type='text'
                      defaultValue={data.exportName}
                      onChange={(ev) => {
                        let val = ev.target.value
                        updateNodeData(id, 'exportName', val)
                      }}
                    />
                  </div>
                </div>
          ...
          {/*
  {
    ...
    style: {
      ...
      height: 100,
            ...
            */}
          <div className={`px-2 py-1 ml-1 mr-3 rounded-xl ${data.isExported ? 'bg-lime-700' : 'bg-gray-100'}`}>
            {data.isExported ? (
