...
import binary from "lib0/dist/binary.cjs";
import buffer from "lib0/dist/buffer.cjs";
 ...
 */
const getCurrentUpdateClock = (db, tableName, docName) =>
  new Promise((resolve, reject) => {
 ...
 */
const getDynamoDbMetaData = (db, tableName, docName) =>
  new Promise((resolve, reject) => {
    const params = {
      TableName: tableName,
      KeyConditionExpression:
 ...
 * @return {Promise<Array<Buffer>>}
 */
const getDynamoDbUpdates = (db, tableName, docName) =>
  new Promise((resolve, reject) => {
    const params = {
      TableName: tableName,
      KeyConditionExpression:
 ...
 * @param {any} key
 */
const dynamoDbDelete = (db, tableName, docName, key) =>
  new Promise((resolve, reject) => {
    const params = {
      Key: {
        ydocname: { S: docName },
 ...
 * @param {any} key
 */
const dynamoDbGet = (db, tableName, docName, key) =>
  new Promise((resolve, reject) => {
    const params = {
      Key: {
        ydocname: { S: docName },
 ...
 */
const dynamoDbPut = (db, tableName, docName, key, val) =>
  new Promise((resolve, reject) => {
    const params = {
      TableName: tableName,
 ...
 * @return {Promise<void>}
 */
const clearRange = (db, tableName, docName, gte, lt) =>
  new Promise((resolve, reject) => {
    // Get all items
...
const DynamoDbPersistence = (config) => {
  const db = config.dynamoDBClient; // new AWS.DynamoDB(config.aws);
  ...
  return {
    ...
    storeUpdate: (docName, update, outsideTransactionQueue) => {
      const callback = (db, config) =>
        storeUpdate(db, config.tableName, docName, update);

      return outsideTransactionQueue
        ? callback(db, config)
        : transact(callback);
    },
    ...
    getStateVector: (docName, outsideTransactionQueue) => {
      const callback = async (db, config) => {
        ...
        if (sv !== null && clock === curClock) {
          ...
          const updates = await getDynamoDbUpdates(
            db,
            config.tableName,
            docName
          );
    ...
    getMetas: (docName, outsideTransactionQueue) => {
      const callback = async (db, config) => {
        ...
        const metas = new Map();
        data.forEach((v) => {
          metas.set(v.ykeysort[3], buffer.decodeAny(v.value));
        });
        return metas;
      };
...
export { DynamoDbPersistence, PREFERRED_TRIM_SIZE, getDynamoDbUpdates, Y };
